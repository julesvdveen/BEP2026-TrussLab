...
class Bridge:
    ...
    Methods:
        ...
        sweep: returns estimated resistance and node position vector for initialized bridge
               structure, usingeither myDAQ data or self-simulated (noisy) data.
    """
    ...
    def solve_x(self, R_vec=None, x_guess=None, plot=False):
        ...
        if x_guess is None:
            if self.rigid:
                x_guess = self.x_current[2:-2]
            else:
                x_guess = self.x_current[2:-1] # x_(n-1) must also be found!
        else:
            if self.rigid:
                x_guess = x_guess[2:-2]
            else:
    ...
    def initial_position(self, plot=False):
        """
        Calculates the deformed node positions based on the deformation resistance
        ...
        # Construct initial guess
        if self.rigid:
            x_guess = np.full(2*(self.n - 2), 25) # node positions to be calculated.
        else:
            x_guess = np.concatenate((np.full(2*(self.n - 2), 25), [width_0])) # x_(n-1) must also be found!
    ...
    def x_plot(self, x=None, anim=False):
        ...
        if anim and x is not None:
            return animate_truss(x)


    def sweep(self, R_ref, f_logrange=(3, np.log10(2e4)), freqsteps=1500, myDAQ=False, myDAQlog=None, resonance_sweep=False, geo_constraint=False, 
        ...
        if myDAQlog is None:
            ...
            if resonance_sweep:
                ...
                # --- PER-BEAM SENSITIVITY CALCULATION ---
                R_nominal  = self.R_init
                beam_keys  = list(self.library.keys())
                per_beam_peaks = []  # lijst van index-arrays, één per beam
                f_diff_targets = []  # lijst van frequentie-arrays, één per beam

                for k in range(self.k):
                    R_perturbed = R_nominal.copy()
                    R_perturbed[k] *= 1.10  # alleen weerstand k verstoren

                    U_sens_k = self.solve_U(f_arr_base, R_ref, R_vec=R_perturbed, noise=False)
                    dU_k = np.abs(mag_base - np.abs(U_sens_k))

                    peaks_k = np.array([np.argmax(dU_k)])
                    
                    per_beam_peaks.append(peaks_k)
                    f_diff_targets.extend(f_arr_base[peaks_k])
                    print(f"  R_{beam_keys[k]}: dominante frequenties (Hz): {np.round(f_arr_base[peaks_k], 1)}")

                    #Plot de dU_k om te kijken of de piek ook op de resonantiefrequentie zit
                    fig, ax = plt.subplots(figsize=(10, 4))
                    ax.semilogx(f_arr_base, dU_k, color=f'C{k}', label=f'R_{beam_keys[k]}')
                    for f in f_arr_base[peaks_k]:
                        ax.axvline(f, color=f'C{k}', linestyle=':', alpha=0.9, linewidth=1.5, label=f'Dominant: {f:.1f} Hz')
                    ax.set_xlabel('Frequency (Hz)')
                    ax.set_ylabel(r'$\Delta U$ (gevoeligheid)')
                    ax.set_title(f'Sensitivity curve: R_{beam_keys[k]}')
                    ax.legend()
                    ax.grid(True, which='both')
                    plt.show()

                # Combine all indices for the solver
                all_indices = np.unique(np.concatenate([base_indices] + per_beam_peaks))
                f_sweep = f_arr_base[all_indices]

                rel_df = kwargs.get('rel_df', 0.05) # Default to +/- 5% of the center frequency

                # --- UNIFIED FEATURE BANDS MERGING LAYOUT ---
                raw_ranges = []
                for f in f_sweep:
        ...
        if response_comparison_plot:
            ...
            if not resonance_sweep or resonance_sweep and not include_mes:
                ...
                # SMART RECOVERY: Use the live blueprint if available, otherwise reconstruct it from the loaded file data
                ...
                # sharex='col' unites the top and bottom rows vertically per band column
                fig, axs = plt.subplots(2, M, figsize=(3 * M + 2.5, 6), 
                                        sharey=False, sharex='col', squeeze=False, constrained_layout=True)
                
                # Global index counter to slice out corresponding data chunks
                start_idx = 0
                for col, f_seg in enumerate(f_segments):
                    end_idx = start_idx + len(f_seg)
                    
                    f_mes_seg = f_arr[start_idx:end_idx]   # ← echte gemeten frequenties
                    U_mes = U_out_mes[start_idx:end_idx]
                    
                    # Bereken alles op dezelfde as:
                    ...
                    # Calculate differences for this sub-band
                    dG_pred = np.abs(U_base) - np.abs(U_pred)
                    dG_theo = np.abs(U_base) - np.abs(U_theo)
                    dG_mes  = np.abs(U_base) - np.abs(U_mes)
                    
                    dP_pred = np.degrees(np.angle(U_base)) - np.degrees(np.angle(U_pred))
                    dP_theo = np.degrees(np.angle(U_base)) - np.degrees(np.angle(U_theo))
                    dP_mes  = np.degrees(np.angle(U_base)) - np.degrees(np.angle(U_mes))
